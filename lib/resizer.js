...
exports.work = (imgEvent) => {
  ...
  return exports.download(imgEvent.imageUploadPath).then(data => {
    ...
    return exports.resizeAll(file.tmpPath, file.tmpS3Bucket, file.tmpS3Key, file.destPath);
  }).then(resized => {
...
exports.upload = (tmpFile, destPath) => {
...
exports.copy = (tmpBucket, tmpKey, destPath) => {
...
exports.resizeAll = (tmpFile, tmpS3Bucket, tmpS3Key, destPath) => {
  return Q.all(RESIZES.map(size => {
    if (size.key === 'original' && tmpS3Bucket && tmpS3Key) {
      return exports.copy(tmpS3Bucket, tmpS3Key, destPath);
    } else if (size.key === 'original') {
      return exports.upload(tmpFile, destPath);
    } else {
      return exports.resize(tmpFile, size).then(data => {
        return exports.upload(data.path, destPath);
      });
    }
  }));
}
